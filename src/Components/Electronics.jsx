...
export const Electronics = () => {
  ...
  const electronicsapi = async () => {
    try {
      const electronicsres = await axios.get("https://dummyjson.com/products/category/smartphones");
      console.log(electronicsres);
      setElectronicitems(electronicsres.data); 
    } catch (error) {
      console.log(error);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {electronicitems.products?.map((item) => (
          <div
            key={item.id}
